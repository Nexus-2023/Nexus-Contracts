 ...
 */
abstract contract NexusBridgeUserRebase is INexusBridge {
    ...
    function updateExitedValidators(uint256 exitedValidator) external override onlyNexus {
        if(getRewardsEarned() < VALIDATOR_DEPOSIT) revert ValidatorNotExited();
        validatorCount -= exitedValidator;
    }

    function rebase() external {
        uint256 rewards_to_claim = getRewardsEarned() - amountDistributed;
